...
export function HowItWorks() {
  return (
    <section id="how-it-works" className="px-5 sm:px-10 pb-12 sm:pb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-6">
        {STEPS.map((step) => (
          <div key={step.number} className="border-t-2 border-[#1a1a18] pt-6">
            <p className="font-display text-sm text-[#c4c2bd] font-light mb-4">{step.number}</p>
            <p className="text-sm font-medium text-[#1a1a18] mb-2">{step.title}</p>
            <p className="text-sm text-[#6b6b66] font-light leading-relaxed">{step.body}</p>
          </div>
        ))}
      </div>
    </section>
